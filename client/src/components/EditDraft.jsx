...
const EditDraft = ({ draft, onClose, onSave }) => {
  const [title, setTitle] = useState(draft.title || "")
  const [content, setContent] = useState(draft.content || "")
  const [font, setFont] = useState(draft.formatting?.font || fonts[0])
  const [fontSize, setFontSize] = useState(draft.formatting?.fontSize || fontSizes[4])
  const [isBold, setIsBold] = useState(draft.formatting?.isBold || false)
  const [isItalic, setIsItalic] = useState(draft.formatting?.isItalic || false)
  const [isUnderline, setIsUnderline] = useState(draft.formatting?.isUnderline || false)
  const [alignment, setAlignment] = useState(draft.formatting?.alignment || "left")
  ...
  const [letterFormat, setLetterFormat] = useState(letterFormats.find(f => f.id === draft.formatting?.letterFormat) || letterFormats[0])
  const [paperSize, setPaperSize] = useState(paperSizes.find(s => s.id === draft.formatting?.paperSize) || paperSizes[0])
  ...
  }, [title, content, font, fontSize, isBold, isItalic, isUnderline, alignment, letterFormat, paperSize, draft._id, onSave])
  ...
  useEffect(() => {
    if (draft) {
      ...
      if (draft.formatting) {
        setFont(draft.formatting.font || fonts[0])
        setFontSize(draft.formatting.fontSize || fontSizes[4])
        setIsBold(draft.formatting.isBold || false)
        setIsItalic(draft.formatting.isItalic || false)
        setIsUnderline(draft.formatting.isUnderline || false)
        setAlignment(draft.formatting.alignment || "left")
        setLetterFormat(letterFormats.find(f => f.id === draft.formatting.letterFormat) || letterFormats[0])
        setPaperSize(paperSizes.find(s => s.id === draft.formatting.paperSize) || paperSizes[0])
      }
      setLastSaved(new Date(draft.updatedAt || draft.createdAt))
      
      if (contentEditableRef.current) {
        contentEditableRef.current.innerHTML = draft.content || ""
      }
    }
  }, [draft])
          ...
          <div className="p-4 border-b border-gray-200">
            ...
            <div className="flex flex-wrap items-center gap-3 py-2">
              ...
              <div className="ml-auto flex items-center gap-4">
                ...
                <button
                  onClick={onClose}
                  className="flex items-center gap-2 px-4 py-2 bg-gray-500 hover:bg-gray-600 text-white rounded-md text-sm"
                >
                  <X size={16} />
                  Close
                </button>
